...
class Model(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.model = Cfc(
            in_features=num_inputs + num_outputs,
            out_feature=output_length * num_outputs,
        ...
        self.timespan_names = timespan_names
    ...
    def forward(self, x, t):
        mu, sigma = self.model(x, t)
        mu = mu.reshape(mu.size(0), self.output_length, self.num_outputs)
        sigma = sigma.reshape(sigma.size(0), self.output_length, self.num_outputs)
        return mu, sigma
...
class Cfc(nn.Module):
    def __init__(
        ...
        self.mu = nn.Linear(self.out_feature, self.out_feature)
        self.sigma = nn.Linear(self.out_feature, self.out_feature)
    ...
    def forward(self, x, timespans, mask=None):
        ...
        mu = self.mu(readout)
        sigma = torch.nn.functional.softplus(self.sigma(readout))
